...
model_xgb<-xgb.train(data=dtrain,eta=0.01,max_depth=1,objective="binary:logistic",nrounds = 1000,verbose=1,watchlist=watchlist)
model_xgb<-xgb.train(data = dtrain,nrounds = 1000,objective="binary:logistic",eta=0.01,max_depth=1,verbose = 1,watchlist = list(train=dtrain,test=dtest),early_stopping_rounds = 20)
model_xgb
class(Train$Reverse)
